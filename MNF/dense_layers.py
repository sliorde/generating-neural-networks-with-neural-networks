...
from utils_mnf import randmat, zeros_d, ones_d, outer
...
class DenseMNF(Layer):
    ...
    def __init__(self, output_dim, activation=tf.identity, N=1, input_dim=None, flows_q=2, flows_r=2, learn_p=False,
                 use_z=True, prior_var=1., name=None, logging=False, flow_dim_h=50, prior_var_b=1., thres_var=1., epsZ=None, epsW=None,epsB=None,
                 **kwargs):
    ...
    def build(self):
        ...
        self.w,self.b = self.sample_weights()
    ...
    def sample_z(self, size_M=1, sample=True):
        ...
        sample_M = isample_M + tf.sqrt(qm0) * self.epsZ if sample else isample_M
    ...
    def call(self, x, sample=True, **kwargs):
        ...
        output = tf.matmul(x,self.w[0])+self.b[0]
        return output



    def sample_weights(self,batch_size=1,sample=True):
        sample_M, _ = self.sample_z(size_M=batch_size, sample=sample)

        std_mg = tf.clip_by_value(self.get_params_W(), 0., self.thres_var)

        sample_M = tf.expand_dims(sample_M, 2)
        mu_W = tf.expand_dims(self.mu_W,0)
        std_mg = tf.expand_dims(std_mg, 0)
        mu_bias = tf.expand_dims(self.mu_bias, 0)
        logvar_bias = tf.expand_dims(self.logvar_bias, 0)

        w = mu_W*sample_M + std_mg*self.epsW #  tf.random_normal(tf.shape(std_mg))

        b = tf.tile(mu_bias +  tf.clip_by_value(tf.exp(logvar_bias), 0., self.thres_var**2)* self.epsB ,(batch_size,1))     #tf.random_normal(tf.shape(logvar_bias))

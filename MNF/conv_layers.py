...
class Conv2DMNF(Layer):
    ...
    def __init__(self, nb_filter, nb_row, nb_col, input_shape=(), activation=tf.identity, N=1, name=None,
                 ...
                 prior_var=1., prior_var_b=1., flow_dim_h=50, logging=False, thres_var=1., epsZ=None, epsW=None, epsB=None, **kwargs):
    ...
    def call(self, x, sample=True, **kwargs):
        ...
        output = tf.nn.conv2d(x, self.w[0], self.subsample, self.border_mode, use_cudnn_on_gpu=True)+self.b[0]
        return output
    ...
    def sample_weights(self,batch_size=1,sample=True):
        ...
        sample_M = tf.expand_dims(tf.expand_dims(tf.expand_dims(sample_M, 1), 2), 1)
        mu_W = tf.expand_dims(self.mu_W,0)
        var_w = tf.expand_dims(var_w, 0)
        mu_bias = tf.expand_dims(self.mu_bias,0)
        var_b = tf.expand_dims(var_b,0)


        w = mu_W * sample_M + var_w*self.epsW #tf.random_normal(tf.shape(var_w))

        b = mu_bias*sample_M + var_b*self.epsB #tf.random_normal(tf.shape(var_b))
